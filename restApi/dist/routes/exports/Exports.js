...
router.post('/', (req, res) => tslib_1.__awaiter(this, void 0, void 0, function* () {
    try {
        ...
        if (!Object.values(_entities_1.ExportTypes).includes(type)) {
            throw Error('Book type is not supported: ' + type);
        }
        const job = new _entities_1.ExportJob(bookId, type);
        jobs.push(job);
        job.process();
        return res.status(http_status_codes_1.OK).send();
    }
    catch (err) {
        _shared_1.logger.error('Error in export post request', err);
        return res.status(http_status_codes_1.BAD_REQUEST).json({
            error: err.message,
        });
    }
